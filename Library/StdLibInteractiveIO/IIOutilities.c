...
/** Get a character either from the input buffer or from hardware.
    ...
    @param[out]   ReadChar  Pointer where to store read character.

    @retval    -1   An error has occurred.  Reason in errno and EFIerrno.
    @retval    -1   No data is available.
    @retval     1   One wide character has been placed in *ReadChar.
**/
ssize_t
EFIAPI
IIO_GetInChar (
